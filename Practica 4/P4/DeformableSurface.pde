...
public class DeformableSurface 
{
     ...
     for(int i = 0; i <  _numNodesX; i++){
       for(int j = 0; j < _numNodesY; j++){
         if(i == 0 || j == 0 || i == (_numNodesX - 1) || j == (_numNodesY - 1))
           _nodes[i][j] = new Particle(new PVector((i * (_lengthX / _numNodesX) - _lengthX/2), (j * (_lengthY / _numNodesY) - _lengthY/2), surfacePosZ), new PVector(0,0,0), nodeMass, true);       
         else
           _nodes[i][j] = new Particle(new PVector((i * (_lengthX / _numNodesX) - _lengthX/2), (j * (_lengthY / _numNodesY) - _lengthY/2), surfacePosZ), new PVector(0,0,0), nodeMass, false);
       }
     }     
  }
  ...
  void avoidCollision(Ball b, float Ke, float Kd, float maxForce, float breakLengthFactor)
  {
     ...
     */
     for(int i = 0; i < _numNodesX; i++)
       for(int j = 0; j < _numNodesY; j++){
         if(_nodes[i][j]._s.dist(b._s) < b._r)
           _springsCollision.put(_nodes[i][j].getId() + "-" + b.getId(), new DampedSpring(b, _nodes[i][j], Ke, Kd, true, maxForce, breakLengthFactor));
         else
           _springsCollision.remove(_nodes[i][j].getId() + "-" + b.getId());
       }
  }
